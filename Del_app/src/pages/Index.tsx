...
const BottomNav = ({ active, onMission }: { active: "home" | "map"; onMission: () => void }) => (
  <nav className="sticky bottom-0 left-0 right-0 mt-auto px-4 pb-4">
    <div className="glass mx-auto flex items-center justify-around rounded-3xl px-3 py-2 shadow-elevated">
      ...
      <NavBtn icon={Receipt} label="Earnings" />
      <NavBtn icon={User} label="Profile" />
    </div>
  </nav>
);

const NavBtn = ({ icon: Icon, label, active }: { icon: any; label: string; active?: boolean }) => (
  <button className={`flex flex-col items-center gap-0.5 rounded-xl px-3 py-2 text-[10px] font-bold uppercase tracking-wider transition
