...
import { ChevronLeft, Leaf, Plus, Minus, MapPin, Calendar, Star } from "lucide-react";
...
const Product = () => {
  ...
  const { data: initialBatch } = useQuery({
    ...
    queryFn: () => inventoryModule.getBatch(id!),
  });
  ...
  const { data: variants = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const items = await inventoryModule.listBatches({ product_id: initialBatch.product_id });
      return items.map((b: any) => ({
        ...
        image: normalizeImageUrl(b.batch_image || b.base_image || "/logo.png"),
  ...
  const { data: related = [] } = useQuery({
    ...
    enabled: !!product,
    queryFn: async () => {
      const items = await inventoryModule.listBatches({ category_name: product?.category });
  ...
  const realBatchId = Number(product.id);
  const compositeId = initialBatch?.product_id ? `${initialBatch.product_id}-${realBatchId}` : String(realBatchId);
  const qty = items[compositeId]?.qty ?? 0;
        ...
        <div className="mt-3 grid gap-6 md:grid-cols-2">
          ...
          <div>
            ...
            <div className="mt-6 flex items-center gap-3">
              {qty === 0 ? (
                <button 
                  onClick={() => add({ ...product, id: compositeId, batchId: realBatchId })} 
                  className="h-12 flex-1 rounded-full bg-mint text-sm font-semibold text-mint-foreground shadow-soft transition hover:bg-mint/90 active:scale-[0.98]"
                >
                  Add to cart · ₹{product.price}
                </button>
              ) : (
                <div className="flex h-12 flex-1 items-center justify-between rounded-full bg-mint text-mint-foreground shadow-soft">
                  <button onClick={() => setQty(compositeId, qty - 1)} className="grid h-12 w-12 place-items-center"><Minus className="h-4 w-4" /></button>
                  <span className="font-display text-lg font-bold">{qty} in cart</span>
                  <button onClick={() => setQty(compositeId, qty + 1)} className="grid h-12 w-12 place-items-center"><Plus className="h-4 w-4" /></button>
                </div>
              )}
            </div>
