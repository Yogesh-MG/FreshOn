...
import api, { clearAuthTokens, getRefreshToken } from "@/utils/api";
...
const Profile = () => {
  ...
  const { data: orders = [] } = useQuery({
    queryKey: ["my-orders"],
    queryFn: async () => {
      const res = await api.get("/api/orders/orders/");
  ...
  const profileMutation = useMutation({
    mutationFn: (payload: Record<string, unknown>) => api.patch("/api/auth/profile-data/", payload).then((res) => res.data),
    onSuccess: (data) => {
  ...
  const saved = useMemo(() => orders.reduce((sum: number, order: any) => sum + Number(order.discount || 0), 0), [orders]);
