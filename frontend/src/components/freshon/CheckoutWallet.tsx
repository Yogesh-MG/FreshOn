          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (!e.target.checked) {
                ...
                // Auto-use max balance when checked
                handleWalletAmountChange(maxWalletAmount);
              }
            }}
            className="h-5 w-5 accent-mint rounded"
          />
          <Wallet className="h-4 w-4 text-mint" />
          <span className="font-semibold text-sm">Use Wallet Balance</span>
        </label>
        ...
        <div className="space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs font-medium text-muted-foreground">Amount to use</label>
              <button
          ...
          <div>
            <input
              ...
              value={walletAmount}
              onChange={(e) => handleWalletAmountChange(Number(e.target.value))}
              className="w-full accent-mint h-2 rounded-lg cursor-pointer"
            />
          </div>

          {/* Amount Breakdown */}
          <div className="rounded-lg bg-white p-3 space-y-2 text-xs border border-mint-soft">
            <div className="flex justify-between">
              <span className="text-muted-foreground">Wallet payment:</span>
              <span className="font-semibold text-green-600">₹{walletAmount.toFixed(2)}</span>
            </div>
            <div className="flex justify-between border-t border-border pt-2">
              <span className="text-muted-foreground font-semibold">Remaining to pay:</span>
          ...
          {remainingAmount > 0 && (
            <p className="text-[10px] text-muted-foreground italic">
              * You'll pay the remaining ₹{remainingAmount.toFixed(2)} via card/UPI
            </p>
          )}
        </div>
      )}
