    ...
    <div className="fixed inset-0 z-50 flex items-end bg-black/50 sm:items-center justify-center p-4">
      <div className="w-full rounded-t-2xl bg-white p-6 sm:max-w-md sm:rounded-2xl">
        ...
        <div className="mb-6 rounded-xl bg-mint-soft p-4">
          ...
          <p className="text-2xl font-bold text-mint">₹{currentBalance.toFixed(2)}</p>
        </div>
        ...
        {/* Submit */}
        <button
          onClick={() => initiateTopup({ amount: Number(amount) })}
          disabled={isInitiating || isVerifying || !amount || Number(amount) < 100}
